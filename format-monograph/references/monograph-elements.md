...
- `position_policy: preserve_anchor`：表格在正文对象序列、分节和相邻正文中的位置不得改变
...
获批 `front_matter` 将整本书书名置于独立、不显示且不计入页码的书名页，并按调用者批准的 `book_title_format` 排版；理工科教材候选缺省为中文黑体、西文 Times New Roman、22 pt、加粗、水平居中、至少 33 pt 行距、段前后 0，并在书名页分节内垂直居中。V0.5.1/schema 1.5 对目录题名默认只保持：不得创建缺失题名。只有结构图用哈希 locator 明确批准且该对象紧邻获批 TOC 起点时，才可处理一个既有标准题名；精确“目录”仅插入四个 ASCII 半角空格成为“目    录”，已有“目    录”不改文字。该对象使用中文黑体（macOS 交付 profile 为 `STHeiti Medium`）、西文 Times New Roman、18 pt、加粗、非斜体、黑色、居中、段前 0、段后 18 pt、keep-with-next、首行及左右缩进 0。自定义、缺失或未批准题名保持；多个题名候选或多个主 TOC 字段在修改前阻断。Word `TOC` 字段只生成目录条目，不生成该题名，本操作不创建、更新或刷新字段。获批 `block_spacing` 在图表完整内容块后插入一个真实空段落；目标软件重新分页后，若该空段落落在新页页首，则删除。空段落属于批准的派生结构，重复运行不得叠加。
...
图片和表格均必须使用 `position_policy: preserve_anchor`。不得将图片或表格移动到另一段、单元格、表格、分节或正文对象位置；不得通过删除后重新插入来调整尺寸。批准的图片调整只修改原有内嵌图片的显示范围并在原容器中居中，同时保持媒体关系、媒体文件、裁剪状态、宽高比、绘图序号和相邻正文顺序不变。由尺寸变化引起的自然分页变化可以接受，但对象锚点不能变化。

独立图片默认不超过版心宽度的 90%，宽高比不小于 1.6 时可到 100%，且不超过可用页高的 65%。图版表内图片不超过原单元格可用宽度的 95%，同一图片行采用相同显示高度。位图最多放大至原显示尺寸的 125%，且放大后有效分辨率不得低于 220 DPI；来源 DPI 未知或不足时不得放大。矢量图可在批准边界内适配。已有裁剪、浮动环绕、关系缺失、用途不明或普通数据表内图片均保持原状并进入 QA。

## 图片可见性与目录纯文本

图片可见性与图片缩放是两个独立决定。仅含嵌入型图片且无作者文字的段落，如因固定值行距小于图片现有显示高度而被裁切，可经结构映射批准后改为自动单倍行距。表格内还要检查行高；只有简单、无跨行合并、无浮动对象的行，才能把不足的 `exact` 行高改为 `atLeast`。不改变图片宽高、裁剪、环绕、锚点、段落/单元格、表格位置或相邻内容顺序。图片与文字共段、浮动图片、文本框和复杂合并行只报告并 QA。

目录来源必须是获批章题、二至四级标题和明确纳入目录的附录。若所有来源均为无对象污染的标题样式，使用标题样式模式；只要目录范围内存在图片段落、标题共段图片或未批准大纲段落，整份目录改用同一专用标识的 Word 标准复杂纯文本 `TC` 字段。`TC` 只保存标题的可见文字和层级，不复制图片，不改标题正文，页码仍由 Word TOC 自动计算。

目标软件更新后，目录结果必须逐项匹配批准来源的数量、顺序、层级和文字，并具有内部目标和页码。任何图片、VML、OLE、文本框、表格、外部关系、空项、额外项或错序都拒绝回写；不得通过删除或移动正文图片来修复目录。
