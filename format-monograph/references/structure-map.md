...
For schema 1.4, `front_matter` may approve one hashed whole-book title locator, a separate unnumbered and vertically centered title-page section, and an optional `book_title_format`. Legacy maps containing `insert_toc_heading_if_missing=true` remain readable for inspection, but every apply path rejects that request before modifying the document; it requires migration and renewed approval. No supported schema may create a missing TOC heading or use that legacy flag to rewrite an existing heading. Schema 1.5 candidates always set the boolean to false and may add a hashed body-paragraph `toc_heading` locator plus `toc_heading_action=normalize_and_format_standard`. The locator must resolve to the sole candidate immediately before the approved `pagination_sections.toc_start`; multiple candidate headings or multiple main-document TOC fields block before modification. Exact `目录` is normalized only to `目    录` (four ASCII spaces), while an already spaced heading keeps its text. Custom text, an absent locator, or `toc_heading_action=preserve` remains unchanged and is reported without heading text. Formatting applies only to the approved standard object through an isolated derived style; no paragraph is inserted, removed, copied, moved, or merged, and no TOC field is created, updated, or refreshed. The `TOC` field does not create its own heading. `block_spacing` may approve one real empty paragraph after each approved data table and complete approved figure block. These spacer paragraphs are derived structure and must be removed by target-software pagination when they would start a new page.
...
- `front_matter`: separate the approved whole-book title page from the TOC; schema 1.5 can normalize and format one explicitly located existing standard TOC heading but cannot create a missing heading.
