...
For the technical-textbook baseline, approve `pagination_sections` only when the front/TOC and body starts already coincide with exact, unambiguous section boundaries; pagination must not create, delete, or move a section. Start the first numbered front-matter section at decimal 1 and let the TOC continue that sequence; when no earlier approved numbered front matter exists, start the TOC at 1. Restart the body exactly once at decimal 1, then continue through later body and landscape sections. Preserve existing first-page section behavior while ensuring a visible PAGE field on numbered first pages. Require default/odd and even footers with exactly one bare editable PAGE field, right-aligned on odd pages and left-aligned on even pages, in 9 pt SimSun/Times New Roman. Repeated application must not add another field, footer part, relationship, or section change. Only a blank footer or a relationship-free pure PAGE footer may be created or normalized; static digits, publisher or chapter text, logos, tables, drawings, other fields, mixed payload, and unknown relationships are blocking QA. Never treat the number of PAGE fields as the physical page count.

Treat the approved whole-book title as a front-matter object, not a chapter title. Put it alone on an unnumbered page and apply the caller-approved title format; the technical-textbook fallback is centered bold 22 pt Chinese Heiti with Times New Roman for western text, at least 33 pt line spacing, zero paragraph spacing, and vertical centering within the title-page section. Preserve the TOC-title boundary by default and never create a missing heading in any supported schema. A legacy `insert_toc_heading_if_missing=true` request may be read for diagnosis but must fail before apply changes the document. Format only one hashed, caller-approved existing standard heading immediately before the approved TOC start: use the displayed value `目    录` (four ASCII spaces), changing exact `目录` only by inserting those four spaces. Preserve an already spaced heading, every custom heading, and every unapproved heading; multiple heading candidates or multiple main-document TOC fields block before modification. The approved existing heading uses Chinese black-face (`STHeiti Medium` in the shipping macOS profile), Times New Roman for western text, 18 pt, bold, non-italic, black, centered, 0 pt before, 18 pt after, keep-with-next, and zero first-line/left/right indents. The `TOC` field generates entries but not this heading and is not updated by this operation. Restart body pagination at 1 independently only through separately approved pagination behavior. Generated multilevel heading numbers must inherit the same effective Chinese/western fonts, size, and weight as their linked Heading style, and Heading 1-4 paragraphs must have zero first-line indent.
