...
Treat the approved whole-book title as a front-matter object, not a chapter title. Put it alone on an unnumbered page, center it, and make it bold. Start a separate TOC page with a centered bold `目录` heading and restart visible decimal TOC pagination at 1; restart body pagination at 1 independently. For approved technical-textbook data tables, use no shading, no outside left/right borders, no horizontal rules between data rows, 1.0 pt major rules, and 0.5 pt minor rules. In a multi-row header, draw minor horizontal rules only where a header hierarchy actually splits; never cut through a vertically merged cell.

Treat mirror margins versus consecutive physical pages as an explicit layout conflict. Microsoft Word can insert parity blank pages when a mirrored document restarts a new section at page 1. When the caller approves mirror margins plus consecutive title, TOC, and body pages, the Microsoft Word adapter must select the physical odd/even section start that matches the next content page, use an editable calculated PAGE field only when the visible number needs a one-page offset, and correct affected PAGEREF results after the final TOC update. The skill performs this refresh automatically; the caller does not manually update the TOC or page fields. Lock corrected TOC/PAGEREF results for delivery, unlock and rebuild them on the next skill run, and instruct the caller to rerun the skill after manuscript pagination changes. Never silently retain a parity blank page or disable mirror margins.
